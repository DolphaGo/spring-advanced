...
[AspectJ](https://www.eclipse.org/aspectj/) 프레임워크
1. 자바 프로그래밍 언어에 대한 완벽한 관점 지향 확장
2. 횡단 관심사의 깔끔한 모듈화
   - 오류 검사 및 처리
   - 동기화
   - 성능 최적화(캐싱)
...
- AOP를 사용하면 핵심 기능과 부가 기능이 코드 상에서 완전히 분리되어서 관리된다.
...
- 궁금하다면 [Java Instrumentation](https://www.baeldung.com/java-instrumentation)을 검색해보자.
- 참고로 수 많은 모니터링 툴들이 이 방식을 사용한다.
- 이 시점에 애스펙트를 적용하는 것을 **로드 타임 위빙**이라고 한다.

**로드 타입 위빙의 단점** : 로드 타임 위빙은 자바를 실행할 때 특별한 옵션(`java -javaagent`)을 통해 클래스 로더 조작기를 지정해야 하는데, 이 부분이 번거롭고 운영하기 어렵다.

> **런타임 시점(프록시)**

![](/images/2022-05-06-15-11-45.png)
...
- 이렇게하면 최종적으로 프록시를 통해 스프링 빈에 부가 기능을 적용할 수 있다.
- 그렇다. 지금까지 우리가 학습한 것이 바로 프록시 방식의 AOP이다.
- 컴파일 타입 위빙이나, 로드 타입 위빙의 경우 코드 자체를 아예 `.class`에 붙여버리기 때문에 프록시를 만들 필요가 없었지만, 런타임 위빙의 경우엔 프록시를 사용하기 때문에 AOP 기능에 일부 제약이 있다.(생성자 등과 같은 거엔 적용할 수 없다. 항상 다형성이 적용되는 메서드 호출에만 적용할 수가 있다.)
...
- 런타임 시점: 실제 대상 코드는 **그대로 유지가 된다.** 대신에 프록시를 통해 부가 기능이 적용된다. 따라서 항상 프록시를 통해야 부가 기능을 사용할 수 있다. 스프링 AOP는 이 방식을 사용한다.

**AOP 적용 위치, 적용 가능 지점(JoinPoint, 조인 포인트)**

- AOP는 다음과 같은 다양한 위치에서 적용할 수 있다.
...
- **그런데, 프록시를 사용하는 스프링 AOP는 메서드 실행 지점에만 AOP를 적용할 수 있다.**
  - **프록시는 메서드 오버라이딩 개념으로 동작**한다.
  - 따라서 생성자나, static 메서드, 필드 값 접근에는 프록시 개념이 적용될 수 없다.
  - 프록시를 사용하는 **Spring AOP의 Join Point는 메서드 실행으로 제한**된다.
  - 프록시 방식을 사용하는 스프링 AOP는 스프링 컨테이너가 관리할 수 있는 **스프링 빈에만 AOP를 적용**할 수 있다.

> 참고

스프링은 AspectJ 문법을 차용하고, 프록시 방식의 AOP를 적용한다.
...
- 그렇다면, 스프링 AOP 보다는 더 기능이 많은 AspectJ를 직접 사용해서 AOP를 적용하는 것이 좋지 않을까?
...
- 실무에서는 스프링이 제공하는 AOP 기능만 사용해도 대부분의 문제를 해결할 수가 있다.
- 따라서 스프링 AOP가 제공하는 기능을 학습하는 것에 집중하자.

## AOP 용어 정리

![](/images/2022-05-07-05-04-42.png)
...
- 주로 AspectJ 표현식을 사용해서 지정 `ex) execute(* ..)`
- 프록시를 사용하는 스프링 AOP는 메서드 실행 지점으로만 포인트컷으로 선별 가능
...
- 하나의 어드바이스와 하나의 포인트컷으로 구성 (특별한 애스펙트라고 이해하자)
